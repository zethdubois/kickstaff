<script lang="ts">
  ...
  const heroBgY = $derived(heroBgPositionYPct ?? 50);
    ...
    try {
      const res = await fetch("/api/admin/rental-landing", {
        ...
        body: JSON.stringify({
          citySlug: theme,
          landingHeroImageUrl,
          landingHeroBgPositionYPct: heroBgPositionYPct ?? null,
          landingHeadline,
          landingBody,
        }),
      });
...
<div class="rentalLandingFrame" style={frameFontStyle}>
  {#if canEditLanding}
    ...
    >
      <RentalWysiwygGear
        ...
        heroBgPositionYPct={heroBgPositionYPct}
        {headline}
        {body}
      />
      <div class="rentalLandingFrame__heroView">
        {#if hasHero}
          <div
            ...
            style:background-position={`center ${heroBgY}%`}
            aria-hidden="true"
          ></div>
        {:else}
          <div class="rentalLandingFrame__heroPlaceholder">
            No hero image — use the gear to set URL or upload
          </div>
        {/if}
      </div>
    </div>
    ...
    <div
      class="rentalLandingFrame__mainOuter rentalLandingFrame__mainOuter--admin"
    >
      <RentalWysiwygGear
        variant="landing"
        citySlug={theme}
      ...
      >
        {#if editing === "headline"}
          <div class="rentalLandingFrame__editor">
            <label class="rentalLandingFrame__label" for="landing-headline"
              >Headline</label
            >
            <input
              id="landing-headline"
        ...
        {:else}
          <div
            class="rentalLandingFrame__editWrap rentalLandingFrame__editWrap--text"
          >
            {#if hasHeadline}
        ...
        {#if editing === "body"}
          <div class="rentalLandingFrame__editor">
            <label class="rentalLandingFrame__label" for="landing-body"
              >Body</label
            >
            <textarea
              id="landing-body"
        ...
        {#if !hasEditorial && editing === null}
          <p class="rentalLandingFrame__fallback">
            No listings iframe or tile links yet. Configure URLs and embedded
            listings under
            <a class="rentalLandingFrame__adminLink" href="/admin/rental-links"
              >Admin → Rental links</a
  ...
  {:else}
    ...
    >
      ...
      {#if !hasEditorial}
        <p class="rentalLandingFrame__fallback">
          No listings or links configured for this city yet. Add a property
          group and/or tile URLs under
          <a class="rentalLandingFrame__adminLink" href="/admin/rental-links"
            >Admin → Rental links</a
          >.
        </p>
...
<style>
  ...
  .rentalLandingFrame {
    ...
    /* Dot stencil: first layer paints on top of --rlf-surface (visible in gutters) */
    --rlf-surface: transparent;
    --rlf-stencil-dot: rgb(0 0 0 / 0.08);
  ...
  :global(.rentalLanding--cda) .rentalLandingFrame {
    --rlf-surface: rgb(15 23 42 / 0.25);
    --rlf-stencil-dot: rgb(255 255 255 / 0.14);
  }
  ...
  :global(.rentalLanding--spt) .rentalLandingFrame {
    --rlf-surface: rgb(15 23 42 / 0.35);
    --rlf-headline-size: clamp(1.2rem, 2.4vw, 1.5rem);
    --rlf-body-size: 0.95rem;
    --rlf-stencil-dot: rgb(255 255 255 / 0.3);
  }
  ...
  :global(.rentalLanding--mos) .rentalLandingFrame {
    ...
    --rlf-stencil-dot: rgb(41 37 36 / 0.11);
  }
