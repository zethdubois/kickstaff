...
<svelte:head>
	<title>Bill runs</title>
</svelte:head>

<div class="monthly">
	<h1 class="monthly__title">Bill runs</h1>
	<p class="monthly__lead">
		Generate a monthly Appfolio CSV from parsed vendor bills (due date in selected month), then
		move it through queue states.
	</p>
			...
			<label class="field">
				...
				<select class="field__input" name="category">
					{#each data.categories as c (c)}
						<option value={c} selected={c === data.defaults.category}>{c}</option>
					{/each}
				</select>
			</label>
			<label class="field">
				...
				{#each data.files as f (f.id)}
					<li class="row">
						...
						<div class="row__actions">
							<a class="btn btn--compact" href={`/tools/bill-runs/files/${f.id}/download`}>Download CSV</a>
							{#if f.status === 'pending'}
