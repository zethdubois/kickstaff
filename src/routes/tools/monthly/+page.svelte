...
<svelte:head>
	<title>Monthly processing</title>
</svelte:head>

<div class="monthly">
	<h1 class="monthly__title">Monthly processing</h1>
	<p class="monthly__lead">
		Generate a monthly Appfolio CSV from parsed utility bills (due date in selected month), then move
		it through queue states.
	</p>
	...
	{/if}
	{#if form?.generated}
		<p class="monthly__ok" role="status">Generated monthly file {form.id} ({form.recordCount} rows).</p>
	{/if}
	...
	<section class="monthly__card">
		<h2 class="monthly__h2">Generate CSV</h2>
		<form method="POST" action="?/generate" class="monthly__form">
			<label class="field">
				<span class="field__label">Vendor</span>
				<input class="field__input" name="vendor" type="text" value={data.defaults.vendor} required />
			</label>
			<label class="field">
				<span class="field__label">City</span>
				<input class="field__input" name="city" type="text" value={data.defaults.city} required />
			</label>
			<label class="field">
				<span class="field__label">Period</span>
				<input class="field__input" name="period" type="month" value={data.defaults.period} required />
			</label>
			<button class="btn" type="submit">Generate monthly CSV</button>
		</form>
	</section>
	...
	<section class="monthly__card">
		...
		<ul class="rows">
			{#if data.files.length === 0}
				<li class="row"><span class="row__muted">No monthly files yet.</span></li>
			{:else}
				{#each data.files as f (f.id)}
					<li class="row">
						<div>
							<div class="row__title">{f.vendor} / {f.period}</div>
							<div class="row__meta">{f.city ?? '—'} · {f.recordCount} rows · {f.createdAt.toLocaleString()}</div>
						</div>
						<div class="row__status">{f.status}</div>
						<div class="row__actions">
							<a class="btn btn--compact" href={`/tools/monthly/files/${f.id}/download`}>Download CSV</a>
							{#if f.status === 'pending'}
								<form method="POST" action="?/markProcessing" class="row__form">
									<input type="hidden" name="id" value={f.id} />
									<button class="btn btn--compact" type="submit">Mark processing</button>
								</form>
							{:else if f.status === 'processing'}
								<form method="POST" action="?/markDone" class="row__form">
									<input type="hidden" name="id" value={f.id} />
									<button class="btn btn--compact" type="submit">Mark done</button>
								</form>
							{/if}
						</div>
					</li>
...
<style>
	.monthly { max-width: 56rem; margin: 0 auto; padding: 1.5rem 1.25rem 3rem; }
	...
	.monthly__err { margin: 0 0 1rem; padding: 0.5rem 0.65rem; border-radius: 10px; background: color-mix(in srgb, #c0392b 12%, transparent); font-size: 0.9rem; white-space: pre-wrap; }
	.monthly__ok { margin: 0 0 1rem; padding: 0.5rem 0.65rem; border-radius: 10px; background: color-mix(in srgb, #1e8449 14%, transparent); font-size: 0.9rem; }
	.monthly__card { margin-bottom: 1.25rem; }
	.monthly__h2 { margin: 0 0 0.65rem; font-size: 1.05rem; }
	.monthly__form { display: grid; gap: 0.75rem; max-width: 26rem; }
