...
<script lang="ts">
  ...
  async function refreshRecentDocs() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          payload.message ?? "Failed to refresh recent documents.",
        );
      }
  ...
  function unitLinkFor(doc: {
    ...
    if (doc.serviceAccountNumber)
      params.set("service_account_number", doc.serviceAccountNumber);
...
<div class="admin">
  ...
  <p class="admin__lead">
    Upload vendor bill PDFs (utility, insurance, taxes, …), then parse each
  ...
  {#if form?.duplicate}
    <p class="admin__ok" role="status">
      Duplicate skipped (existing id: {form.id}).
    </p>
  {/if}
  {#if form?.parsed}
            ...
            <div>
              ...
              {#if d.parseStatus === "parsed"}
                ...
                {:else if d.serviceAccountNumber}
                  <a class="row__mapLink" href={unitLinkFor(d)}
                    >Add bill account (first time)</a
                  >
                {/if}
              {/if}
              {#if d.parseError}
...
<style>
  ...
  .row__error {
    margin-top: 0.2rem;
    font-size: 0.8rem;
    color: color-mix(in srgb, #c0392b 80%, transparent);
  }
  .row__actions {
    margin: 0;
  }
